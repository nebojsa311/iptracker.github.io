let mymap = L.map("mapid").setView([8.82, 32.156], 13);
let marker = L.marker([8.82, 32.156]).addTo(mymap);
...
function getLocationData() {
  axios.get(fullURL ? fullURL : URL).then((data) => {
    ...
    mymap.off();
    mymap.remove();
    mymap = L.map("mapid").setView([data.data.location.lat, data.data.location.lng], 13);
